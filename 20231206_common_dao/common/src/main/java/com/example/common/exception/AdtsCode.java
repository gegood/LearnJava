...
public enum AdtsCode {
    ...
    AdtsCode(HttpStatus httpStatus) {
        this(httpStatus.value(), httpStatus.getReasonPhrase());
    }

    AdtsCode(int value, String reasonPhrase) {
        this.value = value;
    ...
    public static AdtsCode of(int value) {
        return Arrays.stream(AdtsCode.values()).filter(code -> code.value == value).findAny().orElse(null);
    }
